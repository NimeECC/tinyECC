...
interface NN {
  ...
  */
  ...
  command void ModBarrettInit(NN_DIGIT *c, NN_UINT cDigits, Barrett *pbuf);

  command void BarrettSetBuf(Barrett *pbuf);

  command void ModBarrett(NN_DIGIT *a, NN_DIGIT *b, NN_UINT bDigits, NN_DIGIT *c, NN_UINT cDigits);
  ...
  //Computes a = -b mod c
  command void ModNeg(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_UINT digits); 
  //Computes a = (b * c) mod d.
  command void ModMult(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_DIGIT * d, NN_UINT digits);
  //Computes a = (b / c) mod d.
  command void ModDiv(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_DIGIT * d, NN_UINT digits);
  ...
  command void ModDivOpt(NN_DIGIT * a, NN_DIGIT * b, NN_DIGIT * c, NN_DIGIT * d, NN_UINT digits);
  ...
  command int ModSqrRootOpt(NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, NN_UINT digits, NN_DIGIT *omega);
  ...
  command NN_DIGIT TestBit(NN_DIGIT * a, int16_t i);
  //Returns significant length of a in digits.
  command unsigned int Digits(NN_DIGIT * a, NN_UINT digits);
  //Returns 1 iff a = b.
  command result_t Equal(NN_DIGIT * a, NN_DIGIT * b, NN_UINT digits);
  //Returns 1 iff a is even.
  command result_t Even(NN_DIGIT * a, NN_UINT digits);
  //Returns the lucas exponentatiation for the Tate Pairing lucas(2*b,k)/2
  command result_t LucExp(NN_DIGIT * a,NN_DIGIT * b, NN_DIGIT * k,NN_DIGIT * inv2,NN_DIGIT * p,NN_UINT digits);
}
