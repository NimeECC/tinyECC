...
#ifdef PLATFORM_TELOSB  //should implement in assembly
    //width=1
    //accumulator, r9~r11
    //i, r12
    //j, r13
...
*/
 ...
 void NN_Lucas (NN_DIGIT * a,NN_DIGIT * b, NN_DIGIT * k,NN_DIGIT * d,NN_UINT digits) {
   ...
   i = (call NN.Bits(k2,NUMWORDS))-1;
   		
   while (i>-1) {
	if (call NN.TestBit(k2,i)) {
		NN_ModMult(alpha,alpha,a,d,digits); 
		call NN.ModSub(alpha,alpha,b,d,digits); //a=a*beta-P
		call NN.ModSqr(a, a, d, digits);
		call NN.ModSub(a,a,two,d,digits); //beta=beta^2-2
	}
	
	else {
		NN_ModMult(a,alpha,a,d,digits);
		call NN.ModSub(a,a,b,d,digits); //beta=a*beta-P
		call NN.ModSqr(alpha, alpha, d, digits);
		call NN.ModSub(alpha,alpha,two,d,digits); //a=a^2-2
	}
	i--;
   }
 }
...
/* OTHER OPERATIONS */
  ...
  command result_t NN.Even(NN_DIGIT * a, NN_UINT digits)
  {
    return (((digits) == 0) || ! (a[0] & 1));
  }

  //whether a equals to b or not
  command result_t NN.Equal(NN_DIGIT * a, NN_DIGIT * b, NN_UINT digits)
  {
    return (! NN_Cmp (a, b, digits));
  }

  // Return the lucas exponentatiation for the Tate Pairing lucas(2*b,k)/2
  command result_t NN.LucExp(NN_DIGIT * a,NN_DIGIT * b, NN_DIGIT * k,NN_DIGIT * inv2,NN_DIGIT * d,NN_UINT digits) {
...
/*    NN_DIGIT two[NUMWORDS];
   ...
   call NN.ModMult(a,temp2,inv2,d,digits); // lucas(2b,k)*2^(-1)
